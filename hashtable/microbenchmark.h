...
public:
    ...
    static void register_benchmarks(common::contender_list<Benchmark> &benchmarks) {
        auto fill = [](HashTable &map, size_t num, void*) {
        ...
        common::register_benchmark("insert+find", "insert-find", [](HashTable &map, size_t num, void*) {
            for (size_t i = 1; i <= num; ++i) {
        ...
        common::register_benchmark("ins-del-ins + del-ins-del", "ins-del-cycle", [](HashTable &map, size_t num, void*) {
            for (size_t i = 1; i <= num; ++i) {
                // TODO randomize
                map[i] = i;
                map.erase(i);
        ...
        common::register_benchmark("access", "access", fill, [](HashTable &map, size_t num, void*) {
            for (size_t i = 1; i <= num; ++i) {
                (void)map[i];
            }
        }, sizes, benchmarks);

        common::register_benchmark("find", "find", fill, [](HashTable &map, size_t num, void*) {
            for (size_t i = 1; i <= num; ++i) {
                (void)map.find(i);
            }
        }, sizes, benchmarks);
